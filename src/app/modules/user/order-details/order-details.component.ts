  ...
  private getQutotation() {
    this._quotationService.getQuotationById(this.quotationId).subscribe((res) => {
      ...
      this.models.map((model)=>{
          model.order_value <= (model.quantity * model.price) ? this.isOrderValueMatch = true : this.isOrderValueMatch=false;
          model.instant_quote == false ? this.isProcessInstantQuote = false : this.isProcessInstantQuote = true;
          model.require_review == false ? model.require_review = false : model.require_review = true;
      });
